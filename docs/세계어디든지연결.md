...
## 6. 하드웨어 및 클라우드 가이드 (Hardware & Cloud Guide)

시스템 구축에 필요한 핵심 컴포넌트들의 상세 활용 방법입니다.

### 🔌 하드웨어 파트 (IoT Devices)
*   **ESP32-S3 (AI Gateway)**:
    *   **역할**: 농장 전체의 통신 중심점(Gateway) 및 AI 분석 보드.
    *   **설정**: 16MB Flash, 8MB PSRAM 모델을 권장하며, S3-BOX와 같이 디스플레이와 오디오가 통합된 보드를 활용해 음성 명령을 수행할 수 있습니다.
*   **ESP32-P (Mini Node Specialist)**:
    *   **역할**: 배터리 기반 초소형 센서 노드 또는 PICO 시리즈를 이용한 말단 장치.
    *   **설정**: Deep Sleep 모드를 활용해 전력 소비를 극대화하며, ESP-NOW 통신을 통해 게이트웨이로 데이터를 전송합니다.
*   **E-TW11 (Serial-to-Ethernet/WiFi)**:
    *   **역할**: 기존 RS485 기반의 산업용 전동창, 제어기 등을 IP 네트워크로 연결하는 브리지.
    *   **설정**: 9600-8-N-1 시리얼 설정을 통해 하위 제어기와 연결하고, 내부 웹 설정 페이지에서 고정 IP를 부여하여 서버와 통신합니다.

### ☁️ 클라우드 파트 (Infrastructure)
*   **GitHub**:
    1.  레포지토리 생성 후 소스 코드를 푸시합니다.
    2.  `secrets` 탭에 구글 API 키와 같은 민감 정보를 등록하여 보안을 유지합니다.
*   **Docker**:
    1.  `Dockerfile`을 통해 일관된 실행 환경을 정의합니다.
    2.  로컬 테스트 시 `docker-compose up` 명령어로 서버와 데이터베이스 환경을 즉시 구축합니다.
*   **Render**:
    1.  GitHub 레포지토리를 연결하여 **Auto Build** 설정을 활성화합니다.
    2.  환경 변수(Environment Variables)에서 `PORT`와 `DATA_DIR`를 설정하여 다중 농장 인스턴스를 관리합니다.

---

## 7. 결론: 비즈니스 가치 (Business Value)
