...
### 💾 데이터 영구 저장 (Data Management)
*   **월별 데이터 분할**: 시스템은 데이터 비대화를 방지하기 위해 `tsdb_YYYY_MM.csv` 형식으로 매월 새 파일을 생성합니다.
*   **자동 백업 (Google Sheets)**: 10분 주기로 모든 센서 데이터가 구글 스프레드시트에 자동으로 기록됩니다 (설정 시).
*   **데이터 위치**: 모든 데이터는 설정된 `DATA_DIR` 폴더(기본값: `data/`)에 보관됩니다.
...
## 7. 고급 설정 (Advanced Setup)

### 📊 Google Sheets 실시간 연동
1.  **Google Cloud Console**에서 Service Account를 생성하고 JSON 키를 다운로드합니다.
2.  파일명은 `credentials.json`으로 변경하여 프로젝트 루트에 저장합니다.
3.  스프레드시트를 새로 만들고, JSON 파일 안의 `client_email` 주소에 **편집자 권한**으로 공유합니다.
4.  시스템 재시작 시 자동으로 연동이 시작됩니다.

### 📍 다중 농장(Multi-Farm) 운영
환경 변수를 통해 하나의 코드로 여러 농장을 독립적으로 운영할 수 있습니다.
*   `PORT`: 서버 포트 (예: 8001, 8002)
*   `DATA_DIR`: 데이터 저장 폴더 (예: seoul_data, busan_data)
